...
if __name__ == "__main__":
    while True:
        # find latest fish
        fish = HASH_COLLECTION.find_one({"status": "queued"}, sort=[("created_at", -1)])
        print(fish)
        ...
        HASH_COLLECTION.update_one({"_id": fish_id}, { "$set": { "status": "processed", "updated_at": datetime.today().replace(microsecond=0)} })
